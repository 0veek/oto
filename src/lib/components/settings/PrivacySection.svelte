<script lang="ts">
  ...
  async function syncNow() {
    ...
    try { await invoke("set_config", { cfg: config }); status = await invoke<string>("sync_now"); }
    catch (error) { status = `Sync failed: ${String(error)}`; }
    finally { busy = false; }
  }
</script>
...
<section class="space-y-6">
  ...
  <div class="space-y-5 rounded-2xl border border-white/10 bg-white/[0.04] p-6">
    ...
    <label class="block space-y-1.5" class:opacity-50={!config.history_enabled}><span class="text-sm text-slate-300">Maximum entries</span><input type="number" min="1" max="1000" disabled={!config.history_enabled} class="w-full rounded-xl border border-white/10 bg-slate-900 px-3 py-2.5 text-sm" bind:value={config.history_limit} /></label>
  </div>
  <div class="space-y-5 rounded-2xl border border-white/10 bg-white/[0.04] p-6">
